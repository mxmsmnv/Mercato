...
final class MercatoShippingProviderService extends Wire {
    ...
    protected function buildRateContext(MercatoProductList $cart, array $customer, string $provider): array {
        ...
        return ['operation' => 'rates', 'provider' => $provider, 'currency' => MercatoCurrency::normalizeCode((string) $this->commerce->currency), 'origin' => $this->decodeJson((string) ($this->commerce->shipping_provider_origin ?? '')), 'destination' => ['address' => trim((string) ($customer['address'] ?? '')), 'city' => trim((string) ($customer['city'] ?? '')), 'postal_code' => strtoupper(trim((string) ($customer['zip'] ?? ''))), 'country' => strtoupper(trim((string) ($customer['country'] ?? ''))), 'region' => strtoupper(trim((string) ($customer['region'] ?? '')))], 'packages' => $packages, 'declared_value' => round($cart->getSubtotal(), 2), 'timeout_seconds' => max(1, (int) ($this->commerce->shipping_provider_timeout_seconds ?? 5)), 'quote_ttl_seconds' => max(60, (int) ($this->commerce->shipping_provider_quote_ttl_seconds ?? 900))];
    }

...
$_SERVER['HTTP_HOST'] = 'mercato.dev'; $_SERVER['SERVER_NAME'] = 'mercato.dev'; $_SERVER['REQUEST_URI'] = '/products/'; $_SERVER['SCRIPT_NAME'] = '/index.php'; $_SERVER['SCRIPT_FILENAME'] = $site . '/index.php';
...
$redirected = $commerce->seoService()->metadata($product, ['redirect_url' => 'https://mercato.dev/products/replacement/?source=old']); if ($redirected['canonical'] !== 'https://mercato.dev/products/replacement/' || !str_starts_with($redirected['robots'], 'noindex')) throw new \RuntimeException('Redirected product behavior failed.');
$xml = $commerce->seoService()->sitemapXml(); if (!str_contains($xml, '<urlset') || str_contains($xml, '/checkout/') || str_contains($xml, 'token=')) throw new \RuntimeException('Sitemap private-page exclusion failed.');
